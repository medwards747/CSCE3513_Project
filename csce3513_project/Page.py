...
class Page():
    ...
    def updatePlayerInfo(self, team, id, name):
        ...
        if empty_slot_found == True:
            self.team_dictionary[team_color][first_Available] = [
    ...
    def deletePlayer(self, id):
        ...
        if id_found == True:
            ...
            self.team_dictionary[team_color].append(
        ...
        if self.timer_started == FALSE:
            ...
            def update():
                ...
                if self.time_remaining > 0:
                    ...
                    self.page[1]["TopMiddleFrame"]["Label"].config(
            ...
            if data == False:
                ...
                if id_found != True:
                    self.updatePlayerInfo(
                        team, data[0]["id"], data[0]["codename"])
                else:
    ...
    def createTeamEntryPage(self):
        ...
        for k in self.temp_list:
            self.page[1][k]["Frame"] = Frame(
                self.page[0], height=450, width=125)
        ...
        self.player_label_dictionary = {'padx': 2, 'pady': 1, 'bg': "gray40",
                                        'anchor': E, 'bd': 5, 'relief': SUNKEN, 'height': 2, 'font': ("Arial", 10)}
        for k in self.temp_list:
        ...
        self.shared_top_frame_dictionary = {'font': (
            "Arial", 25), 'anchor': CENTER, 'pady': 1, 'padx': 2, 'bd': 5, 'bg': "gray40", 'relief': RAISED}
        ...
        self.mfargs = [["Game Start Button", {"text": "Game Start", "command": self.startGame}],
                       ["Player Entry Button Green", {"text": "Enter Green Player", "command": partial(
                           self.playerEntryPopup, "Green")}],
                       ["Player Entry Button Red", {
                           "text": "Enter Red Player", "command": partial(self.playerEntryPopup, "Red")}],
                       ["Player Deletion Button", {
                           "text": "Remove Player", "command": self.playerRemovalPopup}],
                       ["Clear All Button", {
                           "text": "Clear All Players", "command": self.clearAll}],
                       ["Music Selection Button", {"text": "Change Music", "command": self.runMusicWin}]]
        self.button_options_dictionary = {"pady": 1, "padx": 2, 'bd': 5, 'bg': "gray40",
                                          'fg': "CadetBlue1", 'width': 25, 'height': 2, 'font': ("Arial", 12)}
        # loops through each entry of mfargs and creates then packs the button
        for n in range(0, 6):
            self.page[1]["MiddleFrame"][self.mfargs[n][0]] = Button(
                self.page[1]["MiddleFrame"]["Frame"], **self.mfargs[n][1], **self.button_options_dictionary)
        ...
        music_selection = random.randint(0, 7)
        if self.musicPlayer is not None:
            music_selection = self.musicPlayer.get_selection()
        return self.team_dictionary, music_selection
